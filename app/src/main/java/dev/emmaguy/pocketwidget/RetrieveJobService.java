...
public class RetrieveJobService implements RetrieveCountOfUnreadArticlesAsyncTask.UnreadCountRetrievedListener {
    ...
    //        mRetrieveCountOfUnreadArticlesAsyncTask = new RetrieveCountOfUnreadArticlesAsyncTask(getResources()
    // .getString(R.string.pocket_consumer_key_mobile), accessToken, this, getApplicationContext());
    ...
    private static int getIdOfUnreadCountAtDate(Context c, String date) {
        Cursor cursor = c.getContentResolver()
                .query(DataProvider.UNREAD_ARTICLES_COUNT_URI,
                        new String[] { DataProvider.ID },
                        DataProvider.DATE + " = ? ",
                        new String[] { date },
                        null);
        if (cursor != null) {
